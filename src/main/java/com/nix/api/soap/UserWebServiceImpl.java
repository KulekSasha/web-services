...
import com.nix.api.soap.exception.ServiceExceptionDetails;
...
public class UserWebServiceImpl implements UserWebService {
    ...
    @Override
    public void createUser(User newUser) throws UserWebServiceException {
        Set<ConstraintViolation<User>> validate = validator.validate(newUser);

        if (validate.size() > 0) {
            List<ServiceExceptionDetails> listErrors = new ArrayList<>();
...
//            detail.addDetailEntry(detName1).addTextNode("Error1");
            ...
            validate.forEach(v -> listErrors.add(
                    new ServiceExceptionDetails(v.getPropertyPath().toString(), v.getMessage())));


            UserWebServiceException userWebServiceException = new UserWebServiceException(listErrors);
